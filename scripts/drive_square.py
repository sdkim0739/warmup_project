...
class DriveSquare(object):
    ...
    def rotate(self):
        ...
        while current_angle < ninety_deg:    
            ...
            current_angle = self.velocities.angular.z * (current_time - initial_time) # Angle = angular velocity * time
            r.sleep()
    ...
    def run(self):
        # Until the program is shut down, continue to move the bot forward and rotate it
        while not rospy.is_shutdown():
            self.move_forward()
            rospy.sleep(5) # Allows bot to move forward for some time before rotating
            self.rotate()
